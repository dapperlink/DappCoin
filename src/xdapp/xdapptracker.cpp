// Copyright (c) 2018-2019 The DAPP developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <xdapp/deterministicmint.h>
#include "xdapptracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CxDAPPTracker::CxDAPPTracker(std::string strWalletFile)
{
...
CxDAPPTracker::~CxDAPPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CxDAPPTracker::Init()
{
...
bool CxDAPPTracker::Archive(CMintMeta& meta)
{
...
bool CxDAPPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CxDAPPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CxDAPPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CxDAPPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CxDAPPTracker::GetSerialHashes()
{
...
CAmount CxDAPPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CxDAPPTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CxDAPPTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CxDAPPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CxDAPPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CxDAPPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CxDAPPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CxDAPPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CxDAPPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CxDAPPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CxDAPPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CxDAPPWallet* xDAPPWallet)
{
    bool isxDAPPWalletInitialized = (NULL != xDAPPWallet);
    CMintMeta meta;
    ...
    if (! isxDAPPWalletInitialized)
        xDAPPWallet = new CxDAPPWallet(strWalletFile);
    meta.isSeedCorrect = xDAPPWallet->CheckSeed(dMint);
    if (! isxDAPPWalletInitialized)
        delete xDAPPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CxDAPPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CxDAPPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CxDAPPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CxDAPPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CxDAPPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CxDAPPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CxDAPPWallet* xDAPPWallet = new CxDAPPWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, xDAPPWallet);
        }
        delete xDAPPWallet;
        LogPrint("zero", "%s: added %d dxdapp from DB\n", __func__, listDeterministicDB.size());
    }
