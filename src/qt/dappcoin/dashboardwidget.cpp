...
#include "qt/dappcoin/dashboardwidget.h"
#include "qt/dappcoin/forms/ui_dashboardwidget.h"
#include "qt/dappcoin/sendconfirmdialog.h"
#include "qt/dappcoin/txrow.h"
...
DashboardWidget::DashboardWidget(DAPPGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of DAPP and xDAPP staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareDapp, "square-chart-dapp");
    setCssProperty(ui->labelSquarexDapp, "square-chart-xdapp");
    setCssProperty(ui->labelDapp, "text-chart-dapp");
    setCssProperty(ui->labelXdapp, "text-chart-xdapp");
    ...
    ui->labelAmountXdapp->setText("0 xDAPP");
    ui->labelAmountDapp->setText("0 DAPP");
    setCssProperty(ui->labelAmountDapp, "text-stake-dapp-disable");
    setCssProperty(ui->labelAmountXdapp, "text-stake-xdapp-disable");
    ...
    ui->btnHowTo->setText(tr("How to get DAPP or zDAPP"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("DAPP")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isDapp = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeXDAPP;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair DAPP, xDAPP
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            dapp = (pair.first != 0) ? pair.first / 100000000 : 0;
            xdapp = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalDapp += pair.first;
            chartData->totalXdapp += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesDapp.append(dapp);
        chartData->valuesxDapp.append(xdapp);

        int max = std::max(dapp, xdapp);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("DAPP");
    set1 = new QBarSet("xDAPP");
    ...
    set0->append(chartData->valuesDapp);
    set1->append(chartData->valuesxDapp);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalDapp > 0 || chartData->totalXdapp > 0) {
        setCssProperty(ui->labelAmountDapp, "text-stake-dapp");
        setCssProperty(ui->labelAmountXdapp, "text-stake-xdapp");
    } else {
        setCssProperty(ui->labelAmountDapp, "text-stake-dapp-disable");
        setCssProperty(ui->labelAmountXdapp, "text-stake-xdapp-disable");
    }
    forceUpdateStyle({ui->labelAmountDapp, ui->labelAmountXdapp});
    ui->labelAmountDapp->setText(GUIUtil::formatBalance(chartData->totalDapp, nDisplayUnit));
    ui->labelAmountXdapp->setText(GUIUtil::formatBalance(chartData->totalXdapp, nDisplayUnit, true));
