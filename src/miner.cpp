...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to xdapps that have been in mempool long
                    //and higher priority to xdapps that are large in value
                    ...
                    // xDAPP spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent xDAPP spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This xDAPP serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (pblock->nVersion < 7) {
            if (fZerocoinActive) {
                ...
                if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                    //For the period before v2 activation, xDAPP will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find xDAPP with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the xDAPP key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with xDAPP key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->xdappTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->xdappTracker->ListMints(true, true, true); //update the state
        }
        return error("DAPPMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("DAPPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dappcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins ||
                   (stakingBalance > 0 && nReserveBalance >= stakingBalance));
        ...
        LogPrintf("Running DAPPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("DAPPMiner exception");
    } catch (...) {
        LogPrintf("DAPPMiner exception");
    }

    LogPrintf("DAPPMiner exiting\n");
}
