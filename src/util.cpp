...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "dappcoin" is a composite category enabling all DAPP-related debug output
            if (ptrCategory->count(std::string("dappcoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\DAPP
// Windows >= Vista: C:\Users\Username\AppData\Roaming\DAPP
// Mac: ~/Library/Application Support/DAPP
// Unix: ~/.dappcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DAPP";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "dappcoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty dappcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override dappcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "dappcoind.pid"));
