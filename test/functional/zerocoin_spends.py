...
from test_framework.test_framework import DappcoinTestFramework
...
class ZerocoinSpendTest(DappcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, xdapp_bal, dapp_bal):
            xdapp_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], xdapp_bal)
            dapp_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], dapp_bal)
            return xdapp_bal, dapp_bal
        ...
        xdapp_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(xdapp_balance, 6666)
        ...
        xdapp_balance, balance = check_balances(denom_0, xdapp_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        xdapp_balance, balance = check_balances(denom_2, xdapp_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        xdapp_balance, balance = check_balances(denom_3, xdapp_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-xdapp",
        ...
        _, _ = check_balances(denom_1, xdapp_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
