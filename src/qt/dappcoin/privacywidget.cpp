...
#include "qt/dappcoin/privacywidget.h"
#include "qt/dappcoin/forms/ui_privacywidget.h"
#include "qt/dappcoin/qtutils.h"
#include "guiutil.h"
#include "qt/dappcoin/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(DAPPGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting xDAPP anonymizes your DAPP by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new xDAPP or convert back to DAPP"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 DAPP ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 xDAPP");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 xDAPP");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 xDAPP");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 xDAPP");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 xDAPP");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 xDAPP");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 xDAPP");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 xDAPP");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last xDAPP Movements"));
    ...
    // Only Convert to DAPP enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalxDAPP->setTitleClassAndText("btn-title-grey", "Total 0 xDAPP");
    ui->btnTotalxDAPP->setSubTitleClassAndText("text-subtitle", "Show denominations of xDAPP owned.");
    ui->btnTotalxDAPP->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select DAPP outputs to mint into xDAPP.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent xDAPP");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalxDAPP, SIGNAL(clicked()), this, SLOT(onTotalXdappClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint xDAPP");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DAPP to mint into xDAPP"));
    }else{
        btnText = tr("Convert back to DAPP");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of xDAPP to convert back into DAPP"));
    }
...
void PrivacyWidget::onTotalXdappClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalxDAPP->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalxDAPP->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("xDAPP is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_xDAPP, true)){
        inform(tr("You need to unlock the wallet to be able to %1 xDAPP").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("xDAPP minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackXdapp(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("xDAPP converted back to DAPP"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " xDAPP </b>";
    ...
    ui->btnTotalxDAPP->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
