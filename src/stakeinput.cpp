...
CXDappStake::CXDappStake(const libzerocoin::CoinSpend& spend)
{
...
int CXDappStake::GetChecksumHeightFromMint()
{
...
int CXDappStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CXDappStake::GetChecksum()
{
    return nChecksum;
}

// The xDAPP block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CXDappStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CXDappStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // xDAPP staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CXDappStake::GetUniqueness()
{
    //The unique identifier for a xDAPP is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CXDappStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CXDappStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the xDAPP that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateXDAPPOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create xDAPP output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked xDAPP", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateXDAPPOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create xDAPP output", __func__);
...
bool CXDappStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CXDappStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CxDAPPTracker* xdappTracker = pwallet->xdappTracker.get();
    CMintMeta meta;
    if (!xdappTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    xdappTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!DAPP Stake
bool CDappStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CDappStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CDappStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CDappStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CDappStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CDappStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CDappStake::GetUniqueness()
{
    //The unique identifier for a DAPP stake is the outpoint
...
CBlockIndex* CDappStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
