...
#include "ui_xdappcontroldialog.h"
...
std::set<std::string> XDappControlDialog::setSelectedMints;
std::set<CMintMeta> XDappControlDialog::setMints;

bool CXDappControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == XDappControlDialog::COLUMN_DENOMINATION || column == XDappControlDialog::COLUMN_VERSION || column == XDappControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


XDappControlDialog::XDappControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::XDappControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select xDAPP Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelXDapp->setProperty("cssClass", "text-main-purple");
    ui->labelXDapp_int->setProperty("cssClass", "text-main-purple");
...
XDappControlDialog::~XDappControlDialog()
{
    delete ui;
}

void XDappControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void XDappControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CXDappControlWidgetItem* itemDenom(new CXDappControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CXDappControlWidgetItem *itemMint = new CXDappControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend xDAPP.";
            else if (!mint.isSeedCorrect)
                strReason = "The xDAPP seed used to mint this xDAPP is not the same as currently hold in the wallet";
            else
...
void XDappControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void XDappControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelXDapp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setXDappControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> XDappControlDialog::GetSelectedMints()
{
...
void XDappControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
