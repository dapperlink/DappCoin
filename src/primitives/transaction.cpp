...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between dapp and xdapp
    for(const CTxIn& txin : vin) {
...
bool CTransaction::HasP2DAPPutputs() const
{
    for(const CTxOut& txout : vout) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // DAPP: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)
