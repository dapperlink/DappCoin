...
*Setup instructions for a gitian build of DAPP Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the DAPP
...
to the DAPP GitHub release page.
...
- [Building DAPP Core](#building-dappcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/dappcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/dappcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/dappcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/dappcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/dappcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/dappcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/dappcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (support@dappcoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/dappcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/dappcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/dappcoin-osx-*-build.assert
```
