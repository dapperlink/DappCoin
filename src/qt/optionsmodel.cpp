...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::DAPP);
    ...
    if (settings.contains("nAnonymizeDappcoinAmount") || reset)
        SoftSetArg("-anonymizedappcoinamount", settings.value("nAnonymizeDappcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dappcoin.cpp during shotdown to also remove the window positions
