...
#include "qt/dappcoin/send.h"
#include "qt/dappcoin/forms/ui_send.h"
#include "qt/dappcoin/addnewcontactdialog.h"
#include "qt/dappcoin/qtutils.h"
#include "qt/dappcoin/sendchangeaddressdialog.h"
#include "qt/dappcoin/optionbutton.h"
#include "qt/dappcoin/sendconfirmdialog.h"
#include "qt/dappcoin/myaddressrow.h"
#include "qt/dappcoin/guitransactionsutils.h"
...
SendWidget::SendWidget(DAPPGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("DAPP");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("xDAPP");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (DAPP) or private coins (xDAPP)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("DAPP address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 DAPP");
    ...
    setCssProperty(coinIcon, "coin-icon-dapp");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onDAPPSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onDAPPSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send DAPP");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send xDAPP");
...
void SendWidget::refreshAmounts() {
    ...
    bool isXdapp = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isXdapp));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isXdapp ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendDapp = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendDapp ? AskPassphraseDialog::Context::Send_DAPP : AskPassphraseDialog::Context::Send_xDAPP, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendDapp) ? send(recipients) : sendXdapp(recipients)) {
...
bool SendWidget::sendXdapp(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("xDAPP is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from xDAPP selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!XDappControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = XDappControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendXdapp(
        ...
        inform(tr("xDAPP transaction sent!"));
        XDappControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == XDAPP_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 xDAPP require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one xDAPP transaction
        ...
        emit message("xDAPP transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isDAPP){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any DAPP to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            XDappControlDialog *xDappControl = new XDappControlDialog(this);
            xDappControl->setModel(walletModel);
            xDappControl->exec();
            ui->btnCoinControl->setActive(!XDappControlDialog::setSelectedMints.empty());
            xDappControl->deleteLater();
        } else {
            inform(tr("You don't have any xDAPP in your balance to select."));
        }
    }
}
...
void SendWidget::onDAPPSelected(bool _isDAPP){
    isDAPP = _isDAPP;
    setCssProperty(coinIcon, _isDAPP ? "coin-icon-dapp" : "coin-icon-xdapp");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress dappAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(dappAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(dappAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
