...
public:
    ...
    bool WriteXDAPPSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadXDAPPSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadXDAPPSeed_deprecated(uint256& seed);
    bool EraseXDAPPSeed();
    bool EraseXDAPPSeed_deprecated();

    bool WriteXDAPPCount(const uint32_t& nCount);
    bool ReadXDAPPCount(uint32_t& nCount);
