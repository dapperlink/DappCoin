...
CxDAPPWallet::CxDAPPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing xdapp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadXDAPPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseXDAPPSeed_deprecated()) {
                    LogPrintf("%s: Updated xDAPP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old xdapp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of xdapp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CxDAPPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteXDAPPCount(nCountLastUsed);
    else if (!walletdb.ReadXDAPPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CxDAPPWallet::Lock()
{
    seedMaster = 0;
}

void CxDAPPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CxDAPPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToXDAPP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CxDAPPWallet::LoadMintPoolFromDB()
{
...
void CxDAPPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CxDAPPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CxDAPPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->xdappTracker->HasPubcoinHash(pMint.first)) {
...
bool CxDAPPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToXDAPP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to xdappTracker which also adds to database
    pwalletMain->xdappTracker->Add(dMint, true);
...
void CxDAPPWallet::SeedToXDAPP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CxDAPPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CxDAPPWallet::GenerateDeterministicXDAPP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CxDAPPWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CxDAPPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CxDAPPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
