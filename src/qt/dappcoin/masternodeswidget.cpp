...
#include "qt/dappcoin/forms/ui_masternodeswidget.h"
#include "qt/dappcoin/qtutils.h"
#include "qt/dappcoin/mnrow.h"
#include "qt/dappcoin/mninfodialog.h"

#include "qt/dappcoin/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(DAPPGUI *parent) :
    PWidget(parent),
    ui(new Ui::MasterNodesWidget)
{
    ...
    ui->btnAboutController->setTitleClassAndText("btn-title-grey", "What is a Masternode Controller?");
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:12321 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * 50000)){
            inform(tr("Not enough balance to create a masternode, 50,000 DAPP required."));
            return;
        }
        showHideOp(true);
