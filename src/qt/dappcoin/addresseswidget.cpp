...
#include "qt/dappcoin/forms/ui_addresseswidget.h"
#include "qt/dappcoin/addresslabelrow.h"
#include "qt/dappcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(DAPPGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter DAPP address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress dappAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(dappAdd.Get(), label.toUtf8().constData(),
                dappAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
