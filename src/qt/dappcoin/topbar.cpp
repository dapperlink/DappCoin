...
#include "qt/dappcoin/topbar.h"
#include "qt/dappcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/dappcoin/lockunlock.h"
#include "qt/dappcoin/qtutils.h"
#include "qt/dappcoin/receivedialog.h"
...
TopBar::TopBar(DAPPGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopDapp, ui->labelAmountTopxDapp}, "amount-small-topbar");
    setCssProperty({ui->labelAmountDapp, ui->labelAmountxDapp}, "amount-topbar");
    setCssProperty({ui->labelPendingDapp, ui->labelPendingxDapp, ui->labelImmatureDapp, ui->labelImmaturexDapp}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // DAPP Total
    CAmount dappAvailableBalance = balance + delegatedBalance;
    // xDAPP Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalDapp = GUIUtil::formatBalance(dappAvailableBalance, nDisplayUnit);
    QString totalxDapp = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopDapp->setText(totalDapp);
    ui->labelAmountTopxDapp->setText(totalxDapp);

    // Expanded
    ui->labelAmountDapp->setText(totalDapp);
    ui->labelAmountxDapp->setText(totalxDapp);

    ui->labelPendingDapp->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingxDapp->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureDapp->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturexDapp->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
